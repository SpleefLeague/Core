...
public class InventoryMenuDialogHolderTemplate<B> extends AbstractInventoryMenuTemplate<InventoryMenuDialogHolder<B>> {
    ...
    public InventoryMenuDialogHolder construct(SLPlayer slp) {
        ...
        InventoryMenuDialogHolder menu = new InventoryMenuDialogHolder(is, getTitle(slp), components, staticComponents, super.getAccessController(), super.getVisibilityController(), slp, flags, () -> next.apply(slp).map(i -> i.build()).orElse(null));
        return menu;
    }
